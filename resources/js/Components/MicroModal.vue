...
const emit = defineEmits(['update:cutomerId'])

const setCustomer = e => {
	emit('update:cutomerId', e.id)
	search.value = e.kana
	toggleStatus()
}
...
<template>
	...
	<button @click="searchCustomers" type="button" data-micromodal-trigger="modal-1" class="flex ml-auto text-white bg-indigo-500 border-0 py-1 px-3 focus:outline-none hover:bg-indigo-600 rounded text-lg mt-2">検索する</button>
</template>
